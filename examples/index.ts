import { initialize, useRawApi, useRequestsConfig, useTypedApi } from "../src";
...
initialize({ baseURL, endpoints: Api });
const { interceptors } = useRequestsConfig();
interceptors.onResponse = (r) => {
  console.log(r.status, r.url);
};
interceptors.onError = (res, err) => {};
interceptors.onRequest = (req) => {};

const typed = async () => {
  console.info("✨ Typed requests");
  const { posts, comments } = useTypedApi<typeof Api>();
  const { headers } = useRequestsConfig();

  await posts.get({ params: { id: 1 } });
  await posts.post({});
