...
export class Service<P> {
  ...
  private buildRequest(req: { path?: RequestPath<P> } & RequestInit): Request {
    let baseURL = "";
  ...
  async get<T>(
    ...
    const req = this.buildRequest({ path, method: "GET", ...options });
  ...
  async delete<T>(
    ...
    const req = await this.buildRequest({ path, method: "DELETE", ...options });
    const res = await this.makeRequest(req);
